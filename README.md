...
Array Creation: Demonstrates how to create NumPy arrays from Python lists (np.array()).
n.dim: The number of dimensions (axes) of the array (e.g., 1 for a vector, 2 for a matrix).
n.size: The total number of elements in the array.
n.shape: A tuple indicating the size of the array in each dimension.
...
Basic Array Operations: NumPy arrays support vectorized operations, meaning operations are applied element-wise without explicit loops, leading to significant performance gains.
Scalar*Matrix: (b*2)
